...
function HistoricalChart({ fund, catFundIds, catLabel, histData }) {
  ...
  const searchResults = useMemo(() => {
    ...
    return productSearchFunds.filter(f=>f.fund_id && words.every(w=>f.name.includes(w)) && f.fund_id!==fund.fund_id).slice(0,8);
            ...
            <div style={{ marginTop:8 }}>
              ...
              <div style={{ display:'flex',gap:4,flexWrap:'wrap',marginBottom:6,direction:'rtl' }}>
                {Object.entries(PRODUCT_LABELS).map(([key,{label,icon}])=>(
                  <button key={key} onClick={()=>{ setSearchProduct(key); setSearchQ(''); }}
              ...
              <input autoFocus value={searchQ} onChange={e=>setSearchQ(e.target.value)} placeholder={`חפש ב${PRODUCT_LABELS[searchProduct]?.label||''}…`} style={{ width:'100%',padding:'6px 10px',border:`1px solid ${C.border}`,borderRadius:7,fontSize:12,fontFamily:'inherit',direction:'rtl',outline:'none',background:C.bg }}/>
...
function ComparisonSearch({ allFunds, product, selected, setSelected, onSelectFund }) {
  ...
  // המוצר שממנו מחפשים — מתחיל מהמוצר הנוכחי, אך ניתן לשנות לכל מוצר אחר
