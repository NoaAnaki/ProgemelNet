...
function HistoricalChart({ fund, catFundIds, histData, onClose }) {
  ...
  const W = 480, H = 175, PAD = { t:12, r:10, b:28, l:44 };
    ...
    <div style={{ padding:'0 0 4px' }}>
      ...
      <div style={{ padding:'8px 14px 4px', position:'relative' }}>
        <svg ref={svgRef} width="100%" viewBox={`0 0 ${W} ${H}`}
          ...
          {avgSeries.length > 0 && <path d={seriesPath(avgSeries)} fill="none" stroke={C.chartGray} strokeWidth="1.5" strokeDasharray="5 3"/>}
      ...
      <div style={{ padding:'4px 14px 8px', display:'flex', gap:10, flexWrap:'wrap', alignItems:'center' }}>
        ...
        {avgSeries.length > 0 && (
          <span style={{ display:'flex', alignItems:'center', gap:4, fontSize:10.5, color:C.muted }}>
            <svg width="18" height="8"><line x1="0" y1="4" x2="18" y2="4" stroke={C.chartGray} strokeWidth="1.5" strokeDasharray="4 2"/></svg>
            ממוצע קטגוריה
          </span>
        )}
      </div>
...
function FundTable({ funds, catId, onSelect, selFund }) {
  ...
  function Row({ fund, rank }) {
    ...
    return (
      <tr onClick={() => !isAvg && onSelect(fund)} style={{ background: isAvg ? C.avgBg : isSel ? '#FFF0F3' : C.white, cursor: isAvg ? 'default' : 'pointer', borderBottom:`1px solid #F0EBE6` }}
...
export default function App() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:C.bg, fontFamily:"'Assistant','Heebo',Arial,sans-serif", direction:'rtl' }}>
      ...
      {/* Nav */}
      <nav style={{ background:C.dark, padding:'0 20px', display:'flex', alignItems:'center', justifyContent:'space-between', height:56, position:'sticky', top:0, zIndex:100, boxShadow:'0 2px 10px rgba(0,0,0,0.3)' }}>
        <img src="/logo_white.png" alt="Profit Financial Group" style={{ height:30, objectFit:'contain', display:'block' }}/>
