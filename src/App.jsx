...
const COMPANY_PATTERNS = [
  ...
  { name:'אי.די.אי.',   patterns:['אי.די.אי.','איי.די.איי.','אי.די.','איי. די.','איי.די.'] },
];
...
function TrackBrowser({ product, onSelectFund, selFund, order, funds, onAddToComparison, onAddToChart }) {
  ...
  // פאנל פתוח: נשאר 60vw (אותו רוחב, בלי קפיצה); אם צר מדי הטבלה גוללת אופקית
              ...
              {visibleSheets.map(sh=>(
                <div key={sh} id={`cat-table-${product}-${sh}`}>
                  <FundTable catId={sh} catLabel={sh}
                    ...
                    onAddToComparison={onAddToComparison} onAddToChart={onAddToChart} panelOpen={panelOpen}/>
                </div>
              ))}
            </div>
          </>)}
  ...
  return (
    <div ref={wrapRef} style={{ borderBottom:`1px solid ${C.border}`,background:C.white,padding:'10px 16px 12px' }}>
      ...
      <div style={{ padding:'0' }}>
        ...
        <div style={{ position:'relative',marginBottom:10,maxWidth:`min(60vw, 600px)` }}>
          <input
            value={query}
        ...
        {selected.length>0&&(
          <div style={{ overflowX: panelOpen?'auto':'visible',WebkitOverflowScrolling:'touch',border:`1px solid ${C.border}`,borderRadius:8,...compContainerStyle }}>
...
// ─── מדד מומנטום: גיוס נטו אבסולוטי (₪) ב-3 החודשים האחרונים ─────────────────
// סך הגיוס נטו בשקלים (בנטרול תשואה) ב-N החודשים האחרונים. יחידות: מיליוני ₪ (כמו assets).
function netFlow3mAbs(fundId, months=3) {
...
// מוצרים "חמים" 🔥 בתוך קטגוריה — 20% העליונים לפי גיוס נטו אבסולוטי ב-3ח.
// כללי הלקוח: קטגוריה כולה בגיוס שלילי → אין אש לאף אחד; ואש רק אם הקרן עצמה חיובית.
...
function fmtFlow(v){ return Math.abs(v)>=1000 ? (v/1000).toFixed(1)+' מיליארד ₪' : Math.round(v)+' מ\' ₪'; }
...
function FundTable({ funds, catId, catLabel, onSelect, selFund, selCatId, onAddToComparison, onAddToChart, panelOpen, getCategoryFunds }) {
  ...
  // מפת מומנטום 🔥 יחסית-לקטגוריה: לכל מסלול מוצג — האם הוא מבין 20% העליונים בגיוס נטו 3ח בקטגוריה שלו.
  // תצוגת קטגוריה: הקטגוריה = funds. תצוגת מנהל: כל מסלול מדורג מול קבוצת הקטגוריה שלו (getCategoryFunds).
  const hotMap = useMemo(()=>{
    const result = new Map();
    if(getCategoryFunds){
      const cache = new Map(); // חתימת-קטגוריה → Map חמים, כדי לא לחשב פעמיים
      funds.forEach(f=>{
        const id = f.fund_id; if(!id) return;
        const peers = getCategoryFunds(f) || [];
        const sig = peers.map(p=>p.fund_id).join('|');
        let m = cache.get(sig);
        if(!m){ m = categoryHotMap(peers); cache.set(sig, m); }
        if(m.has(id)) result.set(id, m.get(id));
      });
      return result;
    }
    return categoryHotMap(funds); // תצוגת קטגוריה: funds הוא הקטגוריה כולה
  },[funds,getCategoryFunds]);
