...
function MixChart({ fund, catFundIds, catLabel, histData, allFunds }) {
  ...
  const pathD = (series) => {
    ...
    return series.map((p,i)=>{
      // מצא אינדקס period ב-xPeriods
      const xi = xPeriods.indexOf(p.period);
      if(xi===-1) return null;
      const x=toX(xi), y=toY(p.val);
      return `${i===0||xi===xPeriods.indexOf(series[0]?.period)?'M':'L'}${x.toFixed(1)},${y.toFixed(1)}`;
    }).filter(Boolean).join(' ');
  };
  ...
  if(!mainSeries.length) return (
    <div style={{ padding:'28px 20px',textAlign:'center',color:C.muted,fontSize:13 }}>
      <div style={{ fontSize:24,marginBottom:8 }}>📊</div>אין נתוני תמהיל היסטוריים
    </div>
  );
