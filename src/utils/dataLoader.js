/**
 * ProGemel — Data Loader v5
 * תיקון: sheet names תואמים בדיוק לגליונות האקסל
 * "כללי" מדויק — לא "כללית"
 */
...
// ── בדיקת מילה שלמה בדיוק ────────────────────────────────────────────────────
// מחזיר true רק אם המילה מופיעה כמילה עצמאית (לא חלק ממילה אחרת)
function hasExactWord(str, word) {
  const re = new RegExp('(^|[\\s\\-—,/])' + word + '($|[\\s\\-—,/])');
  return re.test(str);
}

// ── assignSheet — מתאים בדיוק לגליונות באקסל ─────────────────────────────────
function assignSheet(name, source) {
  ...
  if (source === 'pension') {
    ...
    if (name.includes('עוקב מדדים גמיש') || name.includes('עוקב מדדים - גמיש'))   return prefix + '-עוקב-גמיש';
    ...
    if (name.includes('משולב סחיר'))                                                return prefix + '-משולב-סחיר';
    if (name.includes('אגח סחיר') || name.includes('אג"ח סחיר'))                   return prefix + '-אגח-סחיר';
    ...
    if (name.includes('מיועד לפורשים') || name.includes('מיועד-פורשים'))           return prefix + '-מיועד-פורשים';
    ...
    // ← "כללי" בדיוק: רק מסלולים שהשם שלהם הוא במפורש "כללי", לא "כללית"
    return prefix + '-כללי';
  }

  // ── גמל נט / ביטוח נט / השתלמות ─────────────────────
  ...
  // ← "כללי" בדיוק: ברירת מחדל רק למי שלא סווג אחרת
  return 'כללי';
}
