...
export function computeAvgSeries(fundIds, histData, range) {
  ...
  // Build each fund's cumulative curve over the range, keyed by period.
  // A fund is included only if it has at least one point at/after start
  // AND has a point at (or before the first period of) the range so all
  // curves share the same 0% baseline.
  const perFund = [];
  for (const s of allSeries) {
    const slice = s.filter(p => p.period >= start);
    if (slice.length < 2) continue;          // need a baseline + at least one step
    let cum = 1.0;
    const curve = {};
